...
function sendingRequest(msg, initiator, helper) {
	...
	if (url.contains('http://localhost:3000')) {
		...
		msg2 = msg.cloneAll()
		requestUri = new URI('http://localhost:3000/rest/user/login', false)
		...
		msg2
			.getRequestBody()
			.setBody('email=test@mail.ru&password=password01')
		...
		try {
			...
			print(
				'token ****' + JSON.parse(msg2.getResponseBody()).authentication.token
			)
			...
			org.zaproxy.zap.extension.script.ScriptVars.setGlobalVar(
				'logintoken',
				JSON.parse(msg2.getResponseBody()).authentication.token
			)
		...
		header.setHeader(
			'Authorization',
			'Bearer ' +
				org.zaproxy.zap.extension.script.ScriptVars.getGlobalVar('logintoken')
		)
...
function responseReceived(msg, initiator, helper) {
	...
	if (url.contains('http://localhost:3000')) {
		...
		print(
			'responseReceived called response status = ' +
